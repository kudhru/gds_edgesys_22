...
overlapping_keys = ["".join(random.choices(string.ascii_letters, k=jc_size)) for _ in range(num_overlapping_keys)]
non_overlapping_keys_ds_2 = ["".join(random.choices(string.ascii_letters, k=jc_size)) for _ in range(num_non_overlapping_keys)]
non_overlapping_keys_ds_1 = ["".join(random.choices(string.ascii_letters, k=jc_size)) for _ in range(num_non_overlapping_keys)]
...
for partition in range(num_partitions):
    table_one_df, table_one_file = generate_and_write_data_uniform(
        num_keys, keys_ds_1, min_rpk, max_rpk, 0, file_system_prefix, overlap_fraction, partition, num_partitions, nnjc_size
    )
    table_two_df, table_two_file = generate_and_write_data_uniform(
        num_keys, keys_ds_2, min_rpk, max_rpk, 1, file_system_prefix, overlap_fraction, partition, num_partitions, nnjc_size
    )